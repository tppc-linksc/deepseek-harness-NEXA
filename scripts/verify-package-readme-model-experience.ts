...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/client/ui-remote-control': { kind: 'none', reason: 'The browser settings surface registers no prompt, schema, or model context; the Host bridge owns separately admitted phone input.' },
  ...
  'packages/client/ui-desktop-update': { kind: 'none', reason: 'Browser-side application-update surface; registers nothing model-facing.' },
  ...
  'packages/interaction/remote-control': { kind: 'indirect', reason: 'Authenticated phone actions enter model context only through existing Session input and approval consumers.' },
