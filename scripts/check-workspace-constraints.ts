...
const nexaPublishedRepositoryUrl = 'git+https://github.com/tppc-linksc/deepseek-harness-NEXA.git'
/** Fork-owned release members whose source does not exist in the upstream repository. */
const publishedRepositoryUrlByDirectory: Readonly<Record<string, string>> = {
  'apps/desktop': nexaPublishedRepositoryUrl,
  'packages/bundle/desktop-app': nexaPublishedRepositoryUrl,
  'packages/client/ui-desktop-update': nexaPublishedRepositoryUrl,
}
...
function checkWorkspace({ dir, manifest }: WorkspaceManifest): string[] {
  ...
  if (isPublicLandlockPackage) {
    ...
    const expectedRepositoryUrl = publishedRepositoryUrlByDirectory[dir] ?? publishedRepositoryUrl
    if (manifest.repository?.type !== 'git'
