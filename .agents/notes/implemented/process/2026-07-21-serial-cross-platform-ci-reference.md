...
- **Repeat the complete unit inventory in the macOS Sandbox leg** - rejected because the primary CI aggregate already owns portable unit behavior. Running unrelated process, terminal, gateway, and snapshot suites beside the Seatbelt proof adds timing-sensitive failures without strengthening the Darwin sandbox signal.
...
A sandbox regression visible only to a real host kernel, native macOS path identity, or the packed Landlock install can merge before the master run reports it. That post-merge detection window is accepted in exchange for removing four non-blocking jobs from every pull request; the default branch retains the complete signal without repeating unrelated unit suites on macOS.
