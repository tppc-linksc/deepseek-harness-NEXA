...
> Division of labor: this document = the layering model + the channel-independent RPC protocol; the protocol's Web implementation combines HTTP uplink with the [WebSocket downlink carrier](2026-08-04-websocket-downlink-carrier.md), the [portless desktop carrier](2026-08-14-portless-electron-desktop-carrier.md) owns Electron carriage, and the browser object layer is in the [web client architecture note](2026-07-19-gui-web-client-architecture.md).
...
At the same time the physical channels differ per consumer (browser HTTP/WebSocket, in-process Fetch/SSE, Electron UtilityProcess messages), so we also need a channel-independent message model and a single contract source of truth — "adding a method" and "swapping a carrier" must not entangle each other, and every message on the wire must be type-validatable, observable, and reconcilable.
...
- `apps/` holds the externally exported applications, assembled from Client / Host mixtures.
    ...
    - `apps/desktop` reuses the same Web client packages over a portless Fetch carrier backed by validated Electron UtilityProcess messages.
...
| Carrier layer | `dsh-host-webserver` plus the Connection and Modules `./web` adapters | Web HTTP and upgrade: route registry, Fetch bridge, static fallback, `/api` and plugin-bundle routes, WebSocket downlinks, and `__DSH_BOOT__` index injection | Web (browser access) only; Electron does not mount these rows |
...
1. **Pick a fetch impersonation**: browser same-origin HTTP / in-process `host.handler.fetch` injection / a platform Fetch subclass such as the Electron desktop carrier in the "Subclass table" below.
...
The three existing applications preserve the division: the Web application mounts Host, HTTP carrier, and browser composition; the desktop application mounts the same GUI composition behind its UtilityProcess carrier; and `dsh --profile headless` mounts a direct core runner with zero Host, HTTP, or ports. ACP-class protocol bridges do not follow the client-carrier checklist: they expose core to the external ecosystem and mount directly via `ctx.plugin(entry-point plugin)` without Fetch.
...
| `FetchApiClient` | dsh-client-connection, selected by `apps/desktop` | same-origin Fetch over `dsh://app`; Electron main carries requests through validated UtilityProcess messages | desktop unary calls and SSE downlinks without HTTP or WebSocket listeners |
...
|---|---|
| Packaging by product (a duplicated Web family and Electron family) | Products share Host and Client capabilities; a thin surface bundle carries only composition deltas instead of copying their implementations |
