...
| 承载层 | `dsh-host-webserver` 加 Connection 与 Modules 的 `./web` 适配器 | Web HTTP 与 upgrade：route 注册表、Fetch 桥、静态 fallback、`/api` 与插件 bundle route、WebSocket 下行及 `__DSH_BOOT__` index 注入 | Web（浏览器访问）专用；Electron 不挂载这些配置项 |
