...
> 分工线：本篇 = 分层模型 + 通道无关的 RPC 协议；协议的 Web 实现由 HTTP 上行加 [WebSocket 下行载体](2026-08-04-websocket-downlink-carrier.md)组成，[无端口桌面载体](2026-08-14-portless-electron-desktop-carrier.md)负责 Electron 承载，浏览器对象层见 [Web 客户端架构笔记](2026-07-19-gui-web-client-architecture.md)。
...
同时各消费方的物理通道不同（浏览器 HTTP／WebSocket、进程内 Fetch/SSE、Electron UtilityProcess 消息），还需要一个通道无关的消息模型和单一约定真源，让「加一个方法」「换一种载体」互不牵连，且 wire 上的每条消息可类型校验、可观测、可对账。
...
- `apps/` 作为对外导出的应用入口，可以由 Client / Host 混合组装。
    ...
    - `apps/desktop` 经由基于经过校验的 Electron UtilityProcess 消息的无端口 Fetch 载体，复用同一套 Web client 包。
...
| 承载层 | `dsh-host-webserver` 加 Connection 与 Modules 的 `./web` 适配器 | Web HTTP 与 upgrade：route 注册表、Fetch 桥、静态 fallback、`/api` 与插件 bundle route、WebSocket 下行及 `__DSH_BOOT__` index 注入 | Web（浏览器访问）专用；Electron 不挂载这些条目 |
...
1. **选 fetch 伪造方式**：浏览器同源 HTTP / 进程内 `host.handler.fetch` 注入 / 平台 Fetch 子类，例如下文「子类表」中的 Electron desktop 载体。
2. **在 `apps/` 下写拼装模块**：`startHost()` + 客户端子类 + 该应用私有的信号/打印/退出语义；混合体不建包，拼装写在 app 里。
3. **需要 HTTP 承载才 import `dsh-host-webserver`**，否则零端口。

现有三个应用保持这一区分：Web 应用挂载 Host、HTTP 载体与浏览器组合；desktop 应用在 UtilityProcess 载体之后挂载同一 GUI 组合；`dsh --profile headless` 则挂载直接使用核心服务的 runner，不包含 Host、HTTP 或端口。ACP 类协议桥不遵循 client 载体清单：它把 core 暴露给外部生态，直接通过 `ctx.plugin(入口插件)` 挂载，不使用 Fetch。
...
| `FetchApiClient` | dsh-client-connection，由 `apps/desktop` 选择 | 经 `dsh://app` 的同源 Fetch；Electron main 通过经过校验的 UtilityProcess 消息承载请求 | 无 HTTP 或 WebSocket 监听的桌面 unary 调用与 SSE 下行 |
...
| 放弃项 | 一句话理由 |
|---|---|
| 按产品分包（复制一套 Web 家族与 Electron 家族） | 产品共享 Host 与 Client 能力；薄 surface bundle 只携带组装差异，不复制实现 |
