...
Add `@deepseek-ai/dsh-qrcode-remote` as the Host-owned integration boundary. The Web bundle mounts it as the `qrcode-remote` Cordis entry, so the runtime component inventory exposes that exact component identity. It pins the reviewed NEXA Remote commit, owns the computer identity and peer store, connects to the Relay, creates expiring pairing QR payloads, requires computer-side confirmation, revokes paired phones, and exposes only browser-safe state and actions through the generated `remoteControl` Typert namespace. Its state file is atomically replaced with owner-only `0600` permissions; private identity material and channel keys never cross the Remote boundary.
...
Add `@deepseek-ai/dsh-client-ui-remote-control` as a dedicated settings section named **Remote Control** / **远程操控**. It configures the Relay and computer name, reports connection errors, generates and displays a direct-launch WeChat Mini Program code (or a clearly labeled development fallback), confirms the pending phone on the computer, and revokes devices. The code contains only a short Relay ticket; WeChat server credentials stay in the Relay. Polling exists only while the section is mounted and never returns secrets. The Web bundle and desktop composition that inherits it mount both packages. First launch is opt-in; environment values may seed defaults, while saved preferences become authoritative.
