...
The Mini Program menu control intentionally uses a fixed-size semantic `view` instead of a native WeChat `button`. Stable DevTools `2.02.2608040` exposed a native-button flex regression that enlarged the real hit/layout box, compressed the title, and made the drawer tap unreliable even though handler-only tests passed. The regression gate now checks the fixed hit target and setData-before-refresh ordering, and the open DevTools simulator was used to click the menu, inspect the complete workspace/session drawer, close it, and confirm the compact header again.
