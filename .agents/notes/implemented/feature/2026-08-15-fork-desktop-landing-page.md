...
The documentation projector keeps the complete locale-home body instead of retaining only frontmatter. It still removes the repository-only language switcher, and the VitePress navigation continues to own locale selection. The locale roots use a custom editorial composition inside the default VitePress home layout instead of the default hero, feature cards, or a second application framework. A dark atmospheric hero, restrained type scale, desktop-window illustration, linear download list, and runtime axis keep the product hierarchy visible without turning every fact into a card. The visual atmosphere and illustration are CSS and semantic HTML, so the landing page adds no remote media dependency; motion stops under `prefers-reduced-motion`. Canonical prose remains under `docs/user/`, while `website/` contains only configuration and presentation assets.
