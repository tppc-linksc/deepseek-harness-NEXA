...
默认情况下，桌面主进程每天最多查询一次 NEXA GitHub Releases API，其中同时包含稳定版与预发行版。检查会识别 `nexa-v<版本>` 和兼容旧发行的 `desktop-v<版本>` 标签，忽略 draft，并按语义版本选择最高版本。用户可以在独立的“应用更新”设置页中关闭自动检查或立即手动检查；只有 API 报告的版本高于当前应用时才要求发布清单。发现新版本后，应用会在左下角显示非阻塞提醒，而不是打开原生模态弹窗；用户第一次操作会启动已校验的后台下载，下载完成后的独立操作才把暂存安装包交给操作系统。
...
安装仍是用户明确触发的系统流程。每次交接前都会再次校验暂存文件。macOS 会打开未签名的 DMG 并立即请求退出应用，使用户替换并重新启动前不再有旧进程占用；Windows 会打开 NSIS 安装程序并退出正在运行的应用；Linux 会先赋予 AppImage 执行权限再打开。如果系统无法打开安装包，应用会保持运行，并在应用内提醒中报告错误。应用不会自行替换文件、执行下载的源码，也不会在没有用户确认时安装新版本。
