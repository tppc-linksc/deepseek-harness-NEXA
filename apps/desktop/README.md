...
Installation remains an explicit operating-system flow. The staged file is verified again before every handoff. macOS opens the unsigned DMG and immediately requests application exit, so the running copy is closed before the user replaces it and relaunches. Windows opens the NSIS installer and exits the running application. Linux marks the AppImage executable before opening it. A failed opener leaves the application running and reports the error in the in-app notification. The application never replaces its own files, executes downloaded source code, or installs a release without user approval.
