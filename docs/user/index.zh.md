...
<article class="nexa-feature-card nexa-remote-card"><div class="nexa-feature-heading"><span class="nexa-feature-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><rect x="5" y="2" width="14" height="20" rx="3"></rect><path d="M9 6h6M11 18h2"></path></svg></span><div><small>NEXA Remote</small><h3>远程操控</h3></div></div><p>正式的 <code>qrcode-remote</code> 运行组件会在“设置”旁增加一个紧凑的“连接移动端”图标。点击并使用微信扫描自动生成的小程序码，就会直接打开电脑的移动适配控制界面。你可以查看运行任务、处理授权、提交或终止任务、新建会话、选择或创建电脑工作区并修改低风险设置；所有操作仍由已配对电脑执行。</p><div class="nexa-remote-preview" aria-label="连接移动端示意"><div><small>侧边栏</small><strong>连接移动端</strong><span><i></i> 可以扫码</span><p>点击一次 · 微信扫码直达</p></div><b class="nexa-mini-code" aria-hidden="true"></b></div></article>
