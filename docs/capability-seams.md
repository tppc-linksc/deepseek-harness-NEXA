...
| `ctx.desktopProfiles` | `core` | [`desktop-app`](../packages/bundle/desktop-app) | - | - | - | Publishes the immutable desktop profile name and directory to desktop-aware plugins inside the isolated Electron Host. |
| `ctx.desktopPnpm` | `core` | [`desktop-app`](../packages/bundle/desktop-app) | - | - | - | Runs packaged pnpm and official dsh plugin operations through one managed subprocess tree against the isolated desktop profile. |
