...
flowchart LR
  ...
  svc_desktopProfiles["ctx.desktopProfiles<br/>Desktop profile identity"]
  svc_desktopPnpm["ctx.desktopPnpm<br/>Desktop package operations"]
  ...
  pkg_desktop_app --> svc_desktopPnpm
  pkg_desktop_app --> svc_desktopProfiles
  ...
  svc_remoteControl --> pkg_api_remotes
...
| `ctx.remoteControl` | `core` | [`remote-control`](../packages/interaction/remote-control) | - | [`api-remotes`](../packages/api/remotes) | - | 负责电脑身份、加密 Relay 通道、配对与撤销状态、DSH 指令适配器，以及由 api-remotes 挂载的 Typert 控制面。 |
...
| `ctx.desktopProfiles` | `core` | [`desktop-app`](../packages/bundle/desktop-app) | - | - | - | 向隔离 Electron Host 内的桌面感知插件发布不可变的桌面 profile 名称与目录。 |
| `ctx.desktopPnpm` | `core` | [`desktop-app`](../packages/bundle/desktop-app) | - | - | - | 针对隔离桌面 profile，通过同一受管理子进程树运行打包的 pnpm 与官方 `dsh plugin` 操作。 |
