# @deepseek-ai/dsh-qrcode-remote

[English](README.md) | 中文

DeepSeek Harness NEXA 与 NEXA Remote 的 Host 端集成。Web 与桌面组合将它挂载为 `qrcode-remote` Cordis 条目，因此它会出现在“**运行组件 → 组件清单**”中。它负责电脑身份、加密通道对端密钥、Relay 连接、配对码生命周期、应用控制适配，以及侧边栏“**连接移动端**”操作使用的 `remoteControl` Typert 命名空间。

电脑是唯一执行权威。小程序投影电脑拥有的工作区、会话、消息、工具输出、状态、审批请求和设置，再把经过身份认证的控制请求发回该电脑。它可以请求新建会话、追加或停止任务、注册已有项目目录、在电脑授权根下创建一个项目目录，以及修改 Host 标记为允许远程修改的设置；它不会在手机运行 Agent、调用模型、保存第二份权威会话、接收项目绝对路径或执行工具。手机指令通过 `ctx.apiProxy.sessions.prompt` 进入；DSH 的审批请求通过既有 `approval/request` waterfall 处理，手机不可用时继续交由本地响应器，手机允许也只能产生一次性授权 `allowed-once`。

移动信息流沿用桌面的信息层级。普通消息直接呈现；工具调用只显示紧凑动作以及具体命令、文件或测试目标，参数、过程日志和结果默认折叠，用户需要时再展开。桌面 Markdown 结果通过安全的小程序子集渲染。电脑 `running` 快照决定手机显示发送还是停止，手机停止请求会进入同一个 Session 取消 API。历史页按字节控制在 Relay 帧上限以内，因此大型终端结果不会再让手机长期停留在重连或加载状态。

工作区控制通过能力协商并默认拒绝。Host 只公开位于已配置工作区根下、短时有效的不透明目录引用；每次请求都会重新校验规范路径、设备边界，并排除符号链接、敏感目录和隐藏目录。P0 只允许列目录、注册已有项目或创建单层目录，不提供远程移动、重命名、删除、上传或任意文件读写。设置使用 Host 提供的描述符、风险标签、允许列表、内容摘要与 CAS 修订号；Relay、凭据、授权根和其他仅限本地的设置不能从手机修改。

## 配置

| 键 | 含义 |
|---|---|
| `statePath` | 必填的私有 JSON 状态路径，保存电脑身份、偏好和加密通道对端密钥；文件权限为 `0600`。 |
| `enabled` | 尚无状态文件时采用的初始连接开关，默认 `true`。 |
| `relayUrl` | 托管的 `ws://` 或 `wss://` Relay 地址，默认固定生产端点 `wss://relay.tppc.top`。 |
| `allowCustomRelay` | 允许开发配置显式使用非生产 Relay；默认 `false`，产品界面不提供 Relay 编辑器。 |
| `computerName` | 投影到已连接手机的初始电脑名称。 |

生成的 Remote 为类型化 Host 控制面和诊断提供 `state`、`configure`、`reconnect`、`openPairing`、`confirmPairing` 与 `revoke`。任何方法都不会返回私钥。托管模式忽略浏览器提交的 Relay 变更，也不会显示地址编辑器。打开“连接移动端”会调用 `openPairing`，只授权该次新 challenge，并在过期时刷新。Relay 配置仅服务端可见的微信密钥后，要约会成为直接启动配对路由的小程序码；签名 proposal 精确匹配后由 Host 在后台确认，手机随即打开最近的电脑会话。旧 `NEXA:` payload 只保留为开发诊断回退，不作为产品二维码展示。

## 模型体验

间接产生影响：通过 Host 准入边界验证的手机动作会进入普通 Session 输入或既有审批服务。

#### KV Cache 影响

本包不拥有请求前缀；已准入的 Session 消息与审批结果遵循其既有消费方的缓存行为。

## 已知限制与延期工作

- 本地状态文件已限制权限，但尚未接入 Keychain、DPAPI 或其他操作系统安全存储。
- 微信真机网络、小程序审核与 SOTER 生物审批仍是本 Host 包之外的发布门禁。
- 公网 Relay 需要 TLS 终止和持久化 Redis 配置。本地 Relay 测试必须显式设置 `DSH_REMOTE_RELAY_URL=ws://127.0.0.1:8080` 与 `DSH_REMOTE_ALLOW_CUSTOM_RELAY=1`。
