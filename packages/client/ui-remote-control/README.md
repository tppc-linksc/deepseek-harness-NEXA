...
The Host connection follows authenticated transport state instead of the presence of a socket object. A lost Relay connection schedules bounded exponential reconnection, opening the popover restarts a stale Host, and a successful state refresh clears any transient connection error. Desktop snapshots include every workspace-owned, non-archived primary Session, including a new blank Session. A phone-created Session tells the desktop to refresh its Session baseline before selecting the new identifier, so the empty Session is visible on both clients before its first prompt. Orphan Session records remain absent from both navigation trees, keeping the Mini Program drawer a faithful projection of the computer rather than a separately filtered task list.
