...
import { FetchApiClient } from './fetch-api-client.ts'
...
export { FetchApiClient } from './fetch-api-client.ts'
...
export function apply(ctx: Context): void {
  ...
  const desktop = pageLocation?.protocol === 'dsh:'
  const api: IApiClient = fixtureClient ?? (desktop ? new FetchApiClient() : new WebApiClient())
  ...
  const handle: ConnectionHandle = {
    api,
    isLoopback: desktop || pageLocation === undefined || isLoopbackHostname(pageLocation.hostname),
    hostDescription: {
