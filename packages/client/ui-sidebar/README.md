...
`SidebarRootComponentProps` composes the layout owner share, the global `useSessions` and `useWorkspaces` hooks, the declared brand, `sidebar.workspaces`, `sidebar.footer.action`, `sidebar.settings`, and `sidebar.footer.trailing` child slots, and injected `startSession` plus sidebar-toggle callbacks. There is no plugin store.
...
The foot stacks every full-width `sidebar.footer.action` registrant in its own one-column grid row above a bottom-pinned footer row. That final row gives `sidebar.settings` the remaining width and places compact `sidebar.footer.trailing` icon actions at its right edge, so NEXA Remote can sit beside Settings instead of adding another navigation row. Direct children cannot widen beyond the column. Each registrant owns its control and optional portalled panel; the shell owns the additive geometry and shares only the column state (`wide`).
