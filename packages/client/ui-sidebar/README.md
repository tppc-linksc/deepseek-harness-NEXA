...
`SidebarRootComponentProps` composes the layout owner share, the global `useSessions` and `useWorkspaces` hooks, the declared brand, `sidebar.workspaces`, `sidebar.footer.action`, and `sidebar.settings` child slots, and injected `startSession` plus sidebar-toggle callbacks. There is no plugin store.
...
The foot stacks every `sidebar.footer.action` registrant in its own one-column grid row above the bottom-pinned `sidebar.settings` seat in both expanded and rail layouts. Direct children cannot widen beyond the column, so registrant flex direction, wrapping, and flex-basis rules do not reposition siblings. Each registrant owns its control and optional portalled panel; the shell owns the additive list geometry and shares only the column state (`wide`).
