...
当 launcher 启用用户 patch 监视时，`runProfile` 会通过 `watchUserPatches` 持续应用 `cordis.patch.yml` 的变更（一次性 surface 经由有界关闭 dispose 监视器）。Electron 不会在打包应用中暴露 Cordis 配置 HMR 所需的 Node 内部模块，因此打包后的 Electron Host 会禁用该选项，其 patch 层会在下次桌面启动时生效。即使该文件或其直接父目录不存在，监视器仍会监视确切路径；它会串行处理突发变更，并按调用方的层次顺序重新组合用户 patch（组合包层在下、overlay 在上）。读取失败、解析失败或 Loader 候选被拒时，最后一个可用树会继续运行；HMR 服务记录错误后广播 `hmr/config-update-failed(filename, Error)`，并隔离观察方的失败。上下文 dispose 时会关闭 watcher，并等待进行中的刷新结束。
