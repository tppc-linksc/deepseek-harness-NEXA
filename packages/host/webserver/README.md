...
The package knows no harness concepts and serves no files: the `/api` HTTP bridge and downlink WebSockets are routes owned by the connection Web adapter, plugin bundles and the HMR event stream are routes owned by the modules Web adapter and HMR plugin, and dist serving belongs to the fallback owner. `bridgeFetch()` is the shared `node:http` to WHATWG Fetch adapter for browser route owners; it bounds buffered request bodies and streams responses with cancellation and socket backpressure. The upgrade handler owns the protocol handshake and connection contents; the webserver only delivers the raw socket and request. `host` accepts only `127.0.0.1` (default posture) and `0.0.0.0` (deliberate network exposure). This server serves browsers only; the desktop profile omits it and uses `dsh://app` plus a UtilityProcess message bridge. This package never prints; the URL line belongs to the shell.
