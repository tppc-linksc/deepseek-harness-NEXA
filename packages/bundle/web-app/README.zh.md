...
Web 组合与继承它的桌面组合还会挂载可选启用的 [`remote-control`](../../interaction/remote-control/README.md) Host 服务和对应的[“远程操控”设置分区](../../client/ui-remote-control/README.md)。首次启动默认关闭，除非设置 `DSH_REMOTE_ENABLED=1`；一旦保存设置，持久化值即成为权威来源。也可用 `DSH_REMOTE_RELAY_URL` 和 `DSH_REMOTE_COMPUTER_NAME` 写入初始 Relay 地址与电脑名称。
