...
Web 组合与继承它的桌面组合还会挂载可选启用的 [`qrcode-remote`](../../interaction/qrcode-remote/README.md) Host 组件和对应的[“远程操控”设置分区](../../client/ui-remote-control/README.md)。首次启动默认关闭，除非设置 `DSH_REMOTE_ENABLED=1`；一旦保存设置，持久化值即成为权威来源。生产环境把隐藏端点固定为 `wss://relay.tppc.top`，连接后设置会自动生成并刷新配对码。本地或自托管测试可用 `DSH_REMOTE_RELAY_URL` 写入地址，但必须同时设置 `DSH_REMOTE_ALLOW_CUSTOM_RELAY=1` 才会显示编辑框；`DSH_REMOTE_COMPUTER_NAME` 用于写入初始电脑名称。
