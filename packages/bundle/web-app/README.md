...
The Web and inherited desktop compositions also mount the opt-in [`remote-control`](../../interaction/remote-control/README.md) Host service and its [`Remote Control settings section`](../../client/ui-remote-control/README.md). The first launch remains disabled unless `DSH_REMOTE_ENABLED=1`; a saved setting then becomes authoritative. Relay URL and computer name can also be seeded with `DSH_REMOTE_RELAY_URL` and `DSH_REMOTE_COMPUTER_NAME`.
