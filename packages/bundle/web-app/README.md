...
The Web and inherited desktop compositions also mount the opt-in [`qrcode-remote`](../../interaction/qrcode-remote/README.md) Host component and its [`Remote Control settings section`](../../client/ui-remote-control/README.md). The first launch remains disabled unless `DSH_REMOTE_ENABLED=1`; a saved setting then becomes authoritative. Production fixes the hidden endpoint to `wss://relay.tppc.top`, and Settings automatically creates and refreshes a pairing code after connecting. Local or self-hosted testing may seed `DSH_REMOTE_RELAY_URL` and must set `DSH_REMOTE_ALLOW_CUSTOM_RELAY=1` to expose editing; `DSH_REMOTE_COMPUTER_NAME` seeds the computer name.
