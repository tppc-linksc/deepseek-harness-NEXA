...
The bundle also adds `@deepseek-ai/dsh-client-ui-desktop-update` as a dedicated Application Update settings section and lower-left shell notification. Its browser half calls `/_desktop/update/*` on `dsh://app`; Electron main handles those private routes before Host forwarding, so update discovery, download storage, verification, and installer opening never grant those privileges to a browser plugin or the UtilityProcess. Download and installer handoff remain separate actions, and the macOS handoff exits the running application after the DMG opens successfully.
...
- **Updates require operating-system interaction** — a non-blocking reminder starts a background download only after an explicit action, then a completion action reverifies and opens the current platform installer. Windows exits into NSIS; macOS exits after opening the DMG; Linux opens the AppImage. The application does not replace itself or claim a silent cross-platform restart.
